...
export async function onRequestGet(context) {
  const { env } = context;
  const [photos, published, drafts, stories, featured, homePhotos] = await Promise.all([
    ...
    env.DB.prepare('SELECT COUNT(*) c FROM photos WHERE homepage_order IS NOT NULL').first(),
  ]);
  let homeCount = 0;
  try { homeCount = JSON.parse(featured?.story_ids || '[]').length; } catch {}
  return json({
    photos: photos.c, published: published.c, drafts: drafts.c,
    stories: stories.c, homepage: homeCount, homepage_photos: homePhotos.c,
